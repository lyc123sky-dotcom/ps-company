        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {stats.map((s, i) => (
            <Reveal key={s.label} delay={i * 90}>
              <div className="h-full p-6 sm:p-7 rounded-2xl bg-white border border-[#ededed] hover:border-[#ff1493]/40 hover:shadow-[0_8px_24px_rgba(255,20,147,0.06)] hover:-translate-y-1 transition-all duration-300">
                <s.Icon
                  className={`w-7 h-7 mb-4 ${s.iconColor}`}
                  strokeWidth={1.75}
                />
                <div className="text-4xl sm:text-5xl font-black tracking-tight text-[#0a0a0a] leading-none">
                  {s.kind === "num" ? (
